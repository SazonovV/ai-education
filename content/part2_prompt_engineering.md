...
### 2.8. Human in the Loop: когда не отпускать руль

Агент может работать автономно, но не всё стоит ему доверять без присмотра. Human in the Loop — стратегия, при которой человек остаётся точкой контроля на критичных этапах.

**Три уровня автономии:**

| Уровень | Когда использовать | Пример |
|---|---|---|
| **Full auto** | Рутина, обратимые действия | Рефакторинг, генерация тестов, линтинг |
| **Plan → Approve → Execute** | Значимые изменения | Новая фича, изменение архитектуры |
| **Ask every step** | Критичные операции | Миграции БД, деплой, удаление данных |

**Как это реализовано в инструментах:**

- **Claude Code** — `--permission-mode plan`: агент планирует свободно, но write-операции требуют одобрения
- **Roo Code / Kilo Code** — Architect → Code: архитектор проектирует, передаёт в Code через Boomerang, человек подтверждает переход
- **Cursor** — YOLO-mode vs подтверждение каждой команды (настраивается)

**Human in the Loop через промпт:**

В любом промпте можно явно указать точки контроля:

```
Задача: мигрировать users таблицу с MySQL на PostgreSQL.

1. Сначала покажи план миграции — я проверю
2. Напиши SQL-скрипт — я проверю перед запуском
3. Подготовь rollback-скрипт
4. НЕ запускай миграцию сам — только сгенерируй файлы
```

Но вписывать это в каждый промпт — утомительно. Если правила контроля повторяются, их стоит зафиксировать. Именно для этого существуют Rules.
...
### 3.7. Условные правила в AGENTS.md

AGENTS.md поддерживает условную подгрузку правил — агент сам решает, когда их применять:
...
If you are working on React components, read and follow `docs/react-guidelines.md`.

If the user asks to write or modify tests, read and follow `docs/testing-standards.md`.

If you are modifying database models or migrations, read and follow `docs/db-conventions.md`.

If the task involves API endpoints, read `docs/api-style-guide.md` before writing code.
```

Как это работает: агент читает AGENTS.md при старте, видит условия на естественном языке и подгружает нужный файл, когда условие совпадает с текущей задачей. Это не glob-паттерн, а семантическое условие — агент сам определяет релевантность.

**Преимущества:**
- **Экономия контекста:** вместо 500 строк правил в одном файле — 10 строк условий + детали в отдельных файлах
- **Кросс-инструментность:** работает в Claude Code, OpenCode, Codex — везде, где поддерживается AGENTS.md
- **Гибкость условий:** можно описать любое условие на естественном языке, не ограничиваясь путями файлов
- **Масштабирование:** команда добавляет новые файлы правил, не раздувая основной AGENTS.md

**Ограничения подхода:**
- **Нет гарантии срабатывания:** это просьба на естественном языке, а не enforcement на уровне тулинга. Агент может не проверить условие — в отличие от Cursor `globs`, которые срабатывают автоматически
- **Зависимость от модели:** более слабые модели менее надёжно следуют условным инструкциям
- **Дополнительный шаг:** агент должен сам вызвать инструмент чтения файла — это лишний tool call и расход контекста

> Совет: храните файлы правил в `docs/` или `.agents/rules/` и ссылайтесь на них из AGENTS.md. Так основной файл остаётся коротким навигатором, а не монолитом. Для критически важных правил — дублируйте их в основном файле, а не полагайтесь на условную подгрузку.

### 3.8. Как писать эффективные правила
