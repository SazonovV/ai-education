...
**Ложная продуктивность.** Агент пишет много, но не то, что нужно. Симптом: после каждой итерации — «ещё чуть-чуть, и готово», но конца не видно. Корень: задача не определена в виде проверяемых критериев — агент сам додумывает рамки и каждый раз додумывает иначе.
...
### 2.5 Проверка перед завершением — «доказательства до утверждений»
...
- **Kilo Code:** правило режима + custom MCP tool `verify_completion`, который агент обязан вызывать.
...
Хорошая новость — дисциплину из §2.1-2.5 не обязательно писать с нуля. Есть готовые наборы, которые реализуют brainstorm → spec → plan → TDD → verification как вызываемые скиллы. Их можно ставить, читать, адаптировать и коммитить в репозиторий команды.

**`superpowers` — единый пакет для большинства агентов**

Open-source плагин ([github.com/obra/superpowers](https://github.com/obra/superpowers)), который приносит весь набор практик из §2.1-2.5 в виде авто-триггерящихся скиллов. Ключевые:
...
- **`superpowers:writing-plans`** — формат плана с атомарными шагами, полным кодом в каждом шаге, TDD-циклом и коммитом на каждый шаг (без плейсхолдеров вроде «add error handling»).
...
Все скиллы — это markdown-файлы с YAML-фронтматтером (`name`, `description`). Поле `description` — триггер: когда запрос совпадает с ним, агент вызывает скилл *до* ответа. Получается «дисциплина как код»: правила лежат в файлах, версионируются в git, передаются всей команде одной командой установки плагина.

Плагин официально поддерживается на пяти инструментах:

- **Claude Code** — через marketplace: `/plugin install superpowers@claude-plugins-official`.
- **Cursor** — через marketplace: `/add-plugin superpowers` в Agent-чате.
- **OpenCode** — одна строка в `opencode.json`:
  ```json
  { "plugin": ["superpowers@git+https://github.com/obra/superpowers.git"] }
  ```
  Плагин автоматически ставится через Bun при рестарте, мапит Claude Code tools на нативные OpenCode (`TodoWrite` → `todowrite`, `Task`+субагенты → `@mention`, `Skill` → нативный OpenCode `skill`-tool). Скиллы регистрируются через `experimental.chat.system.transform` хук.
- **Codex** — ручной install по инструкции из `.codex/INSTALL.md` в репозитории плагина.
- **Gemini CLI** — через `gemini-extension.json` в репозитории.

**Для Kilo Code готового пакета нет** — собираешь дисциплину сам через `.kilocodemodes` (YAML в корне проекта с определениями agents, JSON back-compat) или Markdown-агентов в `.kilo/agents/<name>.md` и custom MCP tools. Рабочий паттерн — два агента: `plan` создаёт план без записи в код (этот режим есть из коробки); свой `implementer` работает только по утверждённому плану и обязан вызывать custom MCP tool `verify_completion` перед завершением.

**Свои скиллы рядом с superpowers**

Плагин не мешает держать проектные и личные скиллы рядом. Формат одинаковый — markdown + frontmatter:
...
- **Claude Code:** `~/.claude/skills/<name>/SKILL.md` (личные) или `.claude/skills/` в корне проекта (командные).
- **OpenCode:** `~/.config/opencode/skills/<name>/SKILL.md` (личные) или `.opencode/skills/` (проектные). Приоритет: **project > personal > superpowers**.
- **Cursor:** `.cursor/rules/*.mdc` — свой формат rules с условными триггерами (не совместим с superpowers-скиллами, но закрывает ту же потребность).

Командные скиллы держи в репозитории и синхронизируй через git, чтобы дисциплина была одинаковой у всех.

**Tool-agnostic альтернатива — `AGENTS.md` / `CLAUDE.md` / `GEMINI.md`**

Если команда использует одновременно несколько инструментов (кто-то Claude Code, кто-то Codex, кто-то OpenCode), ставь правила дисциплины в один `AGENTS.md` в корне репозитория — его автоматически подхватывают Claude Code, Codex, OpenCode, Aider и Gemini CLI. Это не «плагин» и не авто-триггер, но базовый уровень «дисциплины как код», который читают все. Хороший минимум, когда нет возможности стандартизировать инструмент на команду.
...
- **Live** — то, что видишь по ходу работы: текущее сообщение агента, текущие вызовы инструментов, их вывод
...
- **Молчаливые упрощения.** Ожидал чтение всего файла — а агент прочитал первые 50 строк. Ожидал запуск всех тестов — а запущен один по имени. Эти срезы редко видны в финальном сообщении, только в трейсе.
...
- **Всплески токенов.** Резкий рост потребления токенов в одной секции — обычно агент зашёл в болото и его контекст забит мусором.
...
**Kilo Code:**
- История agents (code/plan/ask/debug) и подзадач — VS Code panel
...
Одни и те же практики реализованы в каждом инструменте по-своему. Где-то — полноценная поддержка, где-то — обходные пути. Подробные конфиги были в лекциях 3-4; здесь — *как именно инструменты помогают (или мешают) практикам из секций 2-3*.
...
- **«Хочу VS Code-интеграцию и визуальную панель агентов»** → Kilo Code. Native subagents (code/plan/ask/debug) + VS Code UI помогают видеть подзадачи глазами. Для строгости понадобятся workaround'ы (правила агента + custom MCP tools) — закладывайся на то, что часть гарантий придётся компенсировать инфраструктурой команды.

### 4.3 Что приходится достраивать

- **Унифицированной observability для всех инструментов нет.** Если хочешь смотреть трейсы агентов из разных IDE в одном дашборде — это либо ручные hook-логгеры с общим форматом, либо внешний слой типа Langfuse / LangSmith поверх API.
...
### 5.3 Ложное «готово» без доказательств
...
- **Решение:** правило оценки масштаба — spec/plan только для нетривиальных задач (>1 файл, >1 шаг, неочевидный подход). Тривиальное — делать сразу. Строго следить, чтобы дисциплина не превращалась в самоцель.
...
- **Решение:** артефакты передачи контекста на диске. План и спека лежат в репо. Коммиты с осмысленными сообщениями. `AGENTS.md` / `CLAUDE.md` как «инструкция для будущих сессий»: что было сделано, какие решения приняты, чего не делать.
...
4. **«Готово» без доказательств — не готово.** Verification-before-completion локально, CI как независимый верификатор глобально. Они не заменяют друг друга.
5. **Читать трейс важно так же, как читать диф.** Tool calls, повторения, молчаливые упрощения, всплески токенов — всё это сигналы.
...
- **Kilo Code:** создай custom MCP tool `log_action` и правило в `.kilocodemodes` (или в Markdown-агенте `.kilo/agents/logger.md`), обязывающее агента вызывать его перед каждым `Edit`.
...
**Лекция 6 · Context Engineering** — токенизация, контекстное окно, стратегии управления контекстом и memory-системы. Берём то, что узнали про дисциплину и наблюдаемость, и учимся видеть, *сколько* контекста стоит каждое действие.
