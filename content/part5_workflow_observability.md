...
**Для Roo / Kilo Code готового пакета нет** — собираешь дисциплину сам через `.roomodes` (JSON в корне проекта с определениями custom modes) и custom MCP tools. Рабочий паттерн — два режима: `planner` создаёт план без записи в код; `implementer` работает только по утверждённому плану и обязан вызывать custom MCP tool `verify_completion` перед завершением.
